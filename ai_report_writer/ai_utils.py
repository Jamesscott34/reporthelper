...
class ModelFallbackManager:
    ...
    def get_working_model(self, task_type: str, current_model: str) -> str:
        ...
        next_model = get_next_model(
            task_type, current_model, self.failed_models[task_type]
        )

        if next_model:
            logger.info(
                f"Switching from {current_model} to {next_model} for {task_type}"
            )
            return next_model
        else:
    ...
    def record_success(
        self, task_type: str, model: str, performance_metrics: Dict = None
    ):
        """
...
def get_model_info(task_type: str) -> Dict[str, Any]:
    ...
    """
    try:
        from .model_config import MODEL_CATEGORIES, MODEL_METADATA, get_model_list

        models = get_model_list(task_type)
        model_info = {"task_type": task_type, "total_models": len(models), "models": []}
